...
utils::globalVariables(c("scoring.fun", "testChar"))
    ...
    if (doneChoice==1) {
      ...
      if (tclvalue(rbChoice)=='dont')
        ...
        if (new.file!='') {
          ...
          file.exists <- file.exists(nameFile)
          if (file.exists) {
      ...
      if (substr(nameFile, 1, nchar(R.home()))==R.home()) {
        ...
        if (grepl('^TST_', nameFile)) {
          ...
          if (!file.exists(nameFile)) {
            ...
            disk.file <- read.csv2(nameFile, nrows=0)  # read only names
            disk.file.names <- names(disk.file)
            names.needed <- c('id', 'test', 'age', 'sex', 'date', 'obs')
            if(any(disk.file.names != names.needed)) {
              msg <- paste("The file ",
                           nameFile,
                           " has not a valid structure for this test.",
  ...
  on.score <- function() {
    ...
    if (tclvalue(nameFileVar) != "Don't save") {
      ...
      if (file.exists) {  # only for already existent files, cheks congruence
        head.file <- as.character(head(read.csv2(tclvalue(nameFileVar)), 1)[1,2])
        head.scores <- sub('^TST_', '', present.test[['test']])
        if (head.file != head.scores) {
          answerValid <- FALSE # do not write
          msg <- paste('The file were you whish save corresponds to test ',
                       as.character(head(read.csv2(tclvalue(nameFileVar)), 1)[1,2]),
                       '.',
                       sep='')
          tkmessageBox(message=msg, icon='error')
          tclvalue(nameFileVar) <- "Don't save"
      ...
      if (answerValid==FALSE) {
        msg <- paste("Results have not been saved.",
        ...
        if (tclvalue(write.itemsVar)=='Yes') {
          if (file.exists) # if file exists, cheks if items are admited
            if ((length(results.scores)+6)==length(read.csv2(tclvalue(nameFileVar), 1))) {
              ...
              msg <- paste("This file doesn't accept items.",
                           ...
                           if (!file.exists(tclvalue(nameFileVar)))  # if new file, write the vars names
      ...
      add.new.test <- function() {
        ...
        write.test.begining <- function(acronymTest, nameTest, ref, n.items, valid,
          ...
          cat('\n  # "sex", "age", "id", "date.test" & "comm" used if appropiate.', file='tmp', append=TRUE)
        ...
        make.scale.window <- function(scale.num, n.items, valid, trans, equal, graph, name.trans) {
          ...
          tkgrid(frameName, sticky='w')
          tkgrid(tklabel(frameName, text="Enter the name of the scale (optional)"), entry.name, sticky='w')
